...
eventsApp.directive('datePicker', function () {
    return {
        ...
        link: function(scope, element, attrs, controller) {
            var forElement = angular.element(document.querySelector(''+attrs.for) );
            scope.element = forElement;
            forElement.on('focus', function() { scope.$apply(function() { scope.showDatePicker = true;});  });
            angular.element(document.querySelector('body')).on('click', function() { scope.$apply( function() { scope.showDatePicker = false; }) });
            forElement.on('click', function(event) { event.stopPropagation();  });
            angular.element(document.querySelector(".calendar-nav")).on('click', function(event) { event.stopPropagation(); } ); //keep datepicker open
        }
    };
});
